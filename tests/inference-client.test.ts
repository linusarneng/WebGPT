...
describe('InferenceClient', () => {
  ...
  it('does not spawn a worker until initialize() is called', () => {
    ...
    client.initialize('qwen2.5-0.5b-instruct');
    expect(FakeWorker.latest).toBeDefined();
    expect(FakeWorker.latest!.sent[0]).toMatchObject({ type: 'initialize', model: 'qwen2.5-0.5b-instruct' });
  });
...
describe('InferenceClient model selection', () => {
  ...
  it('tells the worker which model to load', () => {
    const { client, worker } = makeClient();
    client.initialize('qwen3-0.6b');
    expect(worker().sent[0]).toEqual({ type: 'initialize', model: 'qwen3-0.6b' });
  });

  it('reports the model the worker actually loaded', () => {
    const { client, worker } = makeClient();
    client.initialize('granite-4.0-350m');
  ...
  it('tears the worker down on reset so the next model starts clean', () => {
    ...
    expect(worker().sent).toEqual([{ type: 'initialize', model: 'qwen3-0.6b' }]);
  });
